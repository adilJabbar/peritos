                    ...
                    @if(count($selected) > 0)
                        <div class="space-x-2">
                            @foreach($selected as $badge)
                                <span class="inline-flex rounded-full items-center py-0.5 pl-2.5 @can($id . '.delete') pr-1 @else pr-2.5 @endcan mb-2 text-sm font-medium bg-indigo-100 text-indigo-700">
                                  {{ $badge->name }}
                                    @can($id . '.delete')
                                      <button wire:click="clear{{$id}}Selected({{ $badge->id }})" type="button" class="flex-shrink-0 h-4 w-4 rounded-full inline-flex items-center justify-center text-indigo-400 hover:bg-indigo-200 hover:text-indigo-500 focus:outline-none focus:bg-indigo-500 focus:text-white">
        ...
        >
            ...
            <div class="mt-1 overflow-auto max-h-40">
                @foreach($options as $key => $value)
                    <button
                        wire:click="add{{$id}}Selected({{ $key }})"
                        type="button"
