...
<th
    {{ $attributes->merge(['class' => 'px-6 py-3 bg-gray-50'])->only(['class', 'style', 'colspan', 'rowspan']) }}
>
    @unless ($sortable)
        <span class="text-left text-xs leading-4 font-medium text-cool-gray-500 uppercase tracking-wider">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="group flex items-center space-x-1 text-left text-xs leading-4 font-medium text-cool-gray-500 uppercase tracking-wider group focus:outline-none focus:underline {{ $centered ? 'mx-auto' : '' }}">
            <span>{{ $slot }}</span>
