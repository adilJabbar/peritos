    ...
    <x-card.card class="divide-y divide-gray-200">
        <x-card.header>
            <x-breadcumb.simple>
                ...
                <!-- Subcontrata -->
                <x-breadcumb.item link="{{ route('my_gabinetes.subcontractor.show', ['gabinete' => $this->subcontractor->gabinete->id, 'subcontractor' => $subcontractor->id]) }}">{{ __($subcontractor->name) }}</x-breadcumb.item>
            </x-breadcumb.simple>
        </x-card.header>
    </x-card.card>

    <div class="divide-gray-200 divide-y space-y-4">
        <livewire:subcontractor.data :subcontractor="$subcontractor" />
    </div>
</div>
