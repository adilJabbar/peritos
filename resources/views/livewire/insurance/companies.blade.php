        ...
        <x-slot name="header">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">
                    {{__('Todas las compañías que se utilicen dentro de la aplicación, deben ser dadas de alta y verificadas por un administrador.')}}
                </p>
            </div>
        ...
        <div class=" space-y-4" >
            ...
            <div class="flex justify-between">
                ...
                <div class="flex justify-between space-x-2">
                    @if (count($companies) > 10)
                        <x-input.group borderless paddingless for="perPage">
                            <x-input.select wire:model="perPage">
                                ...
                                @if(count($companies) > 25)<option value="50">50</option>@endif
                                @if(count($companies) > 50)<option value="100">100</option>@endif
                                @if(count($companies) > 100)<option value="500">500</option>@endif
                            </x-input.select>
                        </x-input.group>
                    @endif
                    ...
                    @can('company:create')
{{--                        <livewire:company.import-companies />--}}
            ...
            <!-- table -->
            @include('partials.insurance.table')

        </div>

    </x-card.card-with-header>

    @include('partials.insurance.company.modal')
    ...
    <form wire:submit.prevent="deleteSelected">
        <x-modal.confirmation wire:model.defer="showDeleteModal">
            ...
            <x-slot name="title">
                {{__('Elminar Compañías')}}
                <x-button.close wire:click="$set('showDeleteModal', false)" />
            </x-slot>

            <x-slot name="content">
                {{ __('¿Estás seguro de que quieres eliminar estas compañías?') }} {{ __('Esta acción no se puede deshacer.') }}
            </x-slot>
