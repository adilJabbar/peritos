<div class="space-y-4">
    ...
    <section class="flex flex-col  antialiased bg-gray-100 text-gray-600 min-h-screen p-4">
        <div class="h-full">
            <!-- Pricing -->
            <div class="max-w-5xl mx-auto" x-data="{ annual: {{ ($is_monthly == 1 ? "false" : "true") }} }">
                <h2 class="text-3xl text-gray-800 font-bold text-center mb-4">{{__('Planes disponibles')}}</h2>
                <!-- Toggle switch -->
                <div class="flex justify-center">
                    <div class="flex items-center space-x-3 mb-8">
                        <div
                            class="text-sm text-gray-500 font-medium min-w-[120px] text-right">{{ __('Ver planes mensuales') }}</div>


                        <label for="toogleA"
                               class="flex items-center cursor-pointer    {{ $currntSubscription && $currntSubscription->contract_length == "yearly" ? "cursor-not-allowed	" :"" }}">
                            <!-- toggle -->
                            <div class="relative">
                                <!-- input -->
                ...
                <div class="grid grid-cols-12 gap-6">
                    ...
                    @foreach($packages as $k=>$price)
                        ...
                        <div
                            class="relative col-span-full md:col-span-4 bg-white shadow-md rounded-sm border border-gray-200 {{ $currntSubscription && $currntSubscription->package_id == $price->id ? "cursor-not-allowed" : "" }}    ">
                                            ...
                                            @if($currntSubscription->contract_length == "monthly" )
                                                ...
                                                @else
                                                    <p><span
                                                            ...
                                                            class="text-xl line-through">{{ money_format_360($price->usd_month, $money_format, 0, true) }}</span>
                                                        <span
                                                            class="text-gray-500 font-medium text-sm">{{__('mensual')}}</span>
                                                    </p>
                                                    <span class="text-2xl"></span><span
                                                        class="text-3xl">{{money_format_360($new_month_amount, $money_format, 0, true) }}</span>
                                                    <span
                                                        class="text-gray-500 font-medium text-sm">{{__('mensual')}}</span>
                                                @endif
                                            @else
