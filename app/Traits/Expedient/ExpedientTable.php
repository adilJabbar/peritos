...
trait ExpedientTable
{
    ...
    public function getRowsQueryProperty()
    {
        return Expedient::query()
            ->when($this->gabinete, fn ($query, $gabinete_id) => $query->where('gabinete_id', $gabinete_id))
            ->when(! $this->gabinete, fn ($query) => $query->whereIn('gabinete_id', auth()->user()->gabinetes->pluck('id')))
