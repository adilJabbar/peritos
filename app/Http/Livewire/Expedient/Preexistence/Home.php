...
class Home extends Component
{
    ...
    public HomePreexistence $preexistence;
    ...
    public function mount($expedient)
    {
        ...
        if ($this->expedient->address->preexistenceable) {
            ...
            $this->preexistence = HomePreexistence::make([
                'riskdetail_id' => '',
                'structure' => '',
                'roof' => '',
                'wall' => '',
...
//        dd($this->riskdetail);
    ...
    {
        if ($this->expedient->policy->product && $this->expedient->policy->product->ramo) {
            if ($this->expedient->policy->product->capitals->count() > 0) {
                $this->availableCapitals = $this->expedient->policy->product->capitals;
            } else {
    ...
    {
        $this->capitals = [];
        if ($this->expedient->policy->product && $this->expedient->policy->capitals) {
            foreach ($this->expedient->policy->capitals as $capital) {
    ...
    {
        return view('livewire.expedient.preexistence.home');
    }
}
